...
class LoadBoxCreator(BoxCreator):
    def __init__(self, data_name=None):
        ...
        data = np.loadtxt(self.data_name, delimiter=',',skiprows=1)
        # Convert array to array of tuples
        self.box_trajs = [[]]
        for row in data:
            row = row.astype(int)
            tup = tuple(row)
            self.box_trajs[0].append(tup)

        #self.box_trajs = [list(map(tuple, row.reshape(-1, 3))) for row in data]
        #print(np.array(self.box_trajs)[0][0])
        #print(self.box_trajs)
